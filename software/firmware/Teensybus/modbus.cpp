...
uint16_t getReadResponseDataSize(uint16_t start_reg, uint16_t end_reg)
{
	...
	{ // check if starting register is within the data type range
		if (end_reg > REGISTER_AR_SIZE + INT_REG_OFFSET)
		{																			   // check if the ending register is past the data type range
...
void modbus_update(void)
{
	...
	if (!packet_complete())
		return;						// check if an entire packet has been received otherwise return, also resolves overflow errors
...
bool packet_complete()
{
	...
	if (buffer_get_data_sz() < full_pkt_sz)
		return false; // make sure we have a full packet
	...
	{ // on crc fail remove first byte from buffer This is the only known incorrect byte
		packetSize = 1;
		pop_packet();
		return false;
	}
